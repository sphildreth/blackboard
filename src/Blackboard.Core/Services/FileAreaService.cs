...
public class FileAreaService : IFileAreaService
{
    ...
    public FileAreaService(IDatabaseManager databaseManager, ILogger logger, string fileStorageBasePath = "files")
    {
        ...
        if (!Directory.Exists(_fileStorageBasePath)) Directory.CreateDirectory(_fileStorageBasePath);
    }
    ...
    public async Task<FileAreaDto> CreateFileAreaAsync(FileAreaDto fileArea)
    {
        ...
        if (!Directory.Exists(areaPath)) Directory.CreateDirectory(areaPath);
    ...
    public async Task<bool> DeleteFileAreaAsync(int areaId)
    {
        ...
        if (result > 0) _logger.Information("Deleted file area {AreaName} (ID: {AreaId})", area.Name, areaId);
        ...
        string[]? tags = null, int page = 1, int pageSize = 20, string? sortBy = null, bool sortDesc = false)
    {
        ...
        foreach (var fileData in files)
            try
            {
                ...
                file.Tags = string.IsNullOrEmpty(tagsJson) ? Array.Empty<string>() : JsonSerializer.Deserialize<string[]>(tagsJson) ?? Array.Empty<string>();
    ...
    public async Task<BbsFileDto> UploadFileAsync(FileUploadDto upload, int uploaderId)
    {
        ...
        if (!await CanUserAccessAreaAsync(uploaderId, upload.AreaId, true)) throw new UnauthorizedAccessException("User does not have upload permission for this area");

        var area = await GetFileAreaAsync(upload.AreaId);
        if (area == null) throw new ArgumentException("File area not found");

        if (upload.FileData.Length > area.MaxFileSize) throw new ArgumentException($"File size exceeds maximum allowed size of {FormatFileSize(area.MaxFileSize)}");
        ...
        var duplicateCount = await _databaseManager.QueryFirstAsync<int>(duplicateCheckSql,
            new { upload.AreaId, Checksum = checksum });

        if (duplicateCount > 0) throw new InvalidOperationException("A file with identical content already exists in this area");

        // Save file to disk
        await File.WriteAllBytesAsync(filePath, upload.FileData);
    ...
    public async Task<Stream?> DownloadFileAsync(int fileId, int userId)
    {
        ...
        if (file == null || !file.IsActive || !file.IsApproved) return null;

        if (!await CanUserAccessAreaAsync(userId, file.AreaId)) throw new UnauthorizedAccessException("User does not have download permission for this area");
    ...
    public async Task<bool> DeleteFileAsync(int fileId, int userId)
    {
        ...
        var canDelete = file.UploaderId == userId || userLevel >= 255; // Admin level

        if (!canDelete) throw new UnauthorizedAccessException("User does not have permission to delete this file");
    ...
    public async Task<bool> ApproveFileAsync(int fileId, int approverId)
    {
        ...
        if (result > 0) _logger.Information("File {FileId} approved by user {ApproverId}", fileId, approverId);
    ...
    public async Task<int> CleanupExpiredFilesAsync()
    {
        ...
        if (result > 0) _logger.Information("Cleaned up {Count} expired files", result);
    ...
    public async Task<int> CleanupOrphanedFilesAsync()
    {
        ...
        if (deletedCount > 0) _logger.Information("Cleaned up {Count} orphaned files from disk", deletedCount);
    ...
    {
        var file = await GetFileAsync(fileId);
        if (file == null || !File.Exists(file.FilePath)) return false;

        var fileData = await File.ReadAllBytesAsync(file.FilePath);
    ...
    public async Task<IEnumerable<string>> GetPopularTagsAsync(int count = 20)
    {
        ...
        foreach (var tagRow in tagRows)
            ...
            {
                var tags = JsonSerializer.Deserialize<string[]>(tagRow) ?? Array.Empty<string>();
                foreach (var tag in tags) tagCounts[tag] = tagCounts.GetValueOrDefault(tag, 0) + 1;
            }
    ...
    private static T GetDynamicProperty<T>(dynamic obj, string propertyName)
    {
        try
        {
            ...
            if (typeof(T) == typeof(int?))
                return (T)(object)(value != null ? Convert.ToInt32(value) : null)!;

            if (typeof(T) == typeof(DateTime?))
                return (T)(object)(value != null ? Convert.ToDateTime(value) : null)!;
